...
class RegistrtionTestCase(APITestCase):

  def test_registration(self):
    data = {"username": "testcase", "email": "test1@localhost.app", "password1": "asdfasdf123", "password2": "asdfasdf123"}
...
class ProfileViewSetTestCase(APITestCase):
  ...
  def test_profile_update_by_owner(self):
    response = self.client.put(reverse("profile-detail", kwargs={"pk": 1}), {"city": "Anchiano", "bio": "Tester"})
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    self.assertEqual(json.loads(response.content), {"id": 1, "user": "davinci", "bio": "Tester", "city": "Anchiano", "avatar": None})
  
  def test_profile_update_by_random_owner(self):
